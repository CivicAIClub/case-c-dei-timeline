        ...
        <ScrollReveal>
          <section>
            ...
            <div className="bg-navy rounded-2xl p-6 lg:p-8">
              <h3 className="font-display text-lg text-cream mb-3">The research record</h3>
              <div className="grid sm:grid-cols-3 gap-4">
                {[
                  {
                    stat: '34.7%',
                    label: 'error rate for facial-analysis systems on darker-skinned women — vs 0.8% for lighter-skinned men.',
                    citation: 'Buolamwini & Gebru, Gender Shades (MIT / Microsoft, 2018)',
                  },
                  {
                    stat: '45%+',
                    label: 'of images in popular training datasets depict subjects from the U.S. and Western Europe.',
                    citation: 'Shankar et al., No Classification Without Representation (NeurIPS, 2017)',
                  },
                  {
                    stat: 'Systemic',
                    label: 'image-generation models reproduce and amplify occupational and racial stereotypes from training data.',
                    citation: 'Bianchi et al., Stanford HAI (2023)',
                  },
                ].map((item, i) => (
                  <div key={i} className="p-4 bg-cream/5 rounded-xl">
                    <div className="font-display text-2xl text-maroon-light font-bold mb-1">{item.stat}</div>
                    <p className="text-xs text-cream/80 font-body leading-relaxed mb-2">{item.label}</p>
                    <p className="text-[10px] text-cream/50 font-body italic">
                      {item.citation}
                    </p>
                  </div>
                ))}
              </div>
              <p className="mt-5 text-[11px] text-cream/50 font-body leading-relaxed">
                Research sources: Buolamwini &amp; Gebru (<em>Proceedings of Machine Learning Research</em>, 2018);
                Shankar et al. (NeurIPS Workshop on Machine Learning for the Developing World, 2017);
                Bianchi, Kalluri, Durmus et al. (Stanford Human-Centered AI Institute, 2023).
                Findings quoted above are approximate and directional; consult original papers for methodology.
              </p>
            </div>
          </section>
        </ScrollReveal>
